...
int main(int numArgs, char** args)
{
 ...
 yarp::os::BufferedPort<Bottle> audioClap;
  Bottle *audioBottle;
  ...
  // We set its name
  std::string local_audio_port_name = "/audioReadLocal:i";
  (audioClap).open(local_audio_port_name.c_str());
  printf("Created port %s to get data from audio processor!!\n\n", local_audio_port_name.c_str());

 // And we try to connect it to the port generating the events
  std::string remote_audio_port_name = "/clapAudioData:o";
  connectionOk = yarp.connect(remote_audio_port_name.c_str(), local_audio_port_name.c_str());

  printf("Waiting for audio port to go alive!!!  %d\n", connectionOk);	
  while (!connectionOk){
    connectionOk = yarp.connect(remote_audio_port_name.c_str(), local_audio_port_name.c_str());
  }
  printf("Connected to iCub's audio port\n");
  }
  ...
  while(run && numIters < 10000000){
	...
	mean_y/=nb_coll;
		...
		if(useAudio){
	        	while(count++<200){
			audioBottle = audioClap.read();
        		printf("Audio on?? %d\n", audioBottle->get(0).asInt());
			}
...
//	cubMotor->moveHead(mean_x, mean_y);
			...
			printf("Nb Collisinos = %d, Mean x = %f, mean y = %f\n", nb_coll, mean_x, mean_y);
			cubMotor->moveEyes(mean_x, mean_y);
		}
		moveEyes = false;
      	}
