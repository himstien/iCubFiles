...
/* 
 * Authors: Himanshu Akolkar
 * Created on: 01-July-2014
*/

/**
 * @file applyReceptiveModule.cpp
 * @brief Implementation of the applyreceptiveFieldModule (see header file).
 */

#include <applyReceptiveFieldModule.h>
...
bool aexGrabberModule::configure(yarp::os::ResourceFinder &rf) {
    ...
    printf("configure in applyReceptiveFieldModule \n");

    //printf("moduleName  %s \n", moduleName.c_str());
    moduleName            = rf.check("name", 
                           Value("/applyReceptiveField"), 
    ...
    */
    retinaName             = rf.check("retina", 
                           Value("/icub/retina0"), 
                           "Device name (string)").asString();
    devicePortName         =  retinaName;
    printf("trying to connect to the DVS at %s \n",devicePortName.c_str());
    ...
    */
    ...
    /*
    * get the file name of file with receptive field weights
    */
    fileWithWeights             = rf.check("weightFile", 
                           Value("none"), 
                           "filename of the weight matrix (string)").asString();
    printf("trying to read %s for weights \n",fileWithWeights.c_str());
    fileWithWeightsFull = rf.findFile(fileWithWeights.c_str());

    /*
    * get the file name of file where 
    */
    dumpNameComplete = "";
