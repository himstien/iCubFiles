...
int main(int numArgs, char** args)
{
  ...
  // File for outputing the data
	std::ofstream output_file; //FILE *output_file;
  	output_file.open(filename.c_str(), std::ofstream::out); //output_file = fopen (filename.c_str(), "wb+rb");
	if (output_file == NULL) 
...
////////////////////////////////////////////////////////////////////////////////////
 ...
 // And we try to connect it to the port generating the events
  	std::string remote_hand_events_port_name = "/icub/cartesianController/left_arm/state:o";
  ...
  // Variables that will hold the coordinates of the collisions
	...
	// If we have both the adress and the timestamp, we update the trackers
				if(address_ev_found && ts_found){
					...
					handPos = handPose.read();
					handPosRight = handEvents.read();
					output_file << 	(*handPos).get(0).asDouble() << " " << (*handPos).get(1).asDouble() << " " << (*handPos).get(2).asDouble() << " " << (*handPosRight).get(0).asDouble() << " " << (*handPosRight).get(1).asDouble() << " " << (*handPosRight).get(2).asDouble(); //  << " " << std::endl;
	
//fprintf(output_file, "%f %f %f %f %f %f", (*handPos).get(0).asDouble(), (*handPos).get(1).asDouble(), (*handPos).get(2).asDouble(), (*handPosRight).get(0).asDouble(), (*handPosRight).get(1).asDouble(), (*handPosRight).get(2).asDouble());
						
					handIzhNeuronLeft.updatePotential((!movingLeft)*1000.0, ev_t);
					handIzhNeuronRight.updatePotential((!movingRight)*1000.0, ev_t);

					output_file << " " << ev_x << " " << ev_y << " " << pol << " " << channel << " " << ev_t << std::endl; 
					//	fprintf(output_file, " %d %d %d %d %f\n", ev_x, ev_y, pol, channel, ev_t);
						...
						if(!movingLeft)
						{
							leftHandWeights[ev_x*128+ev_y] = leftHandWeights[ev_x*128+ev_y] + 50*( (exp(-(ev_t-handIzhNeuronLeft.getLastSpikeTime() )/150))>10000?0:exp(-(ev_t-handIzhNeuronLeft.getLastSpikeTime())/150) );						
						}
						else
						{	
							leftHandWeights[ev_x*128+ev_y] = leftHandWeights[ev_x*128+ev_y] - 70 + 10*exp( -(ev_t-handIzhNeuronLeft.getLastSpikeTime()-ev_t )/150);
						}

						if(!movingRight)
						{
							rightHandWeights[ev_x*128+ev_y] = rightHandWeights[ev_x*128+ev_y] + 50*( (exp(-(ev_t-handIzhNeuronRight.getLastSpikeTime() )/150))>10000?0:exp(-(ev_t-handIzhNeuronRight.getLastSpikeTime())/150) );	
						}
						else
						{	
							rightHandWeights[ev_x*128+ev_y] = rightHandWeights[ev_x*128+ev_y] - 70 + 10*exp( -(ev_t-handIzhNeuronRight.getLastSpikeTime() )/150);	
						}


						image_hand_weights.pixel(ev_y, ev_x).r = (leftHandWeights[ev_x*128+ev_y]>0)?leftHandWeights[ev_x*128+ev_y]:0;
//						double temp = image_hand_weights.pixel(ev_y, ev_x).r - image_hand_weights.pixel(ev_y, ev_x).b;
//						image_hand_weights.pixel(ev_y, ev_x).r = temp;
...
//						temp = image_hand_weights.pixel(ev_y, ev_x).b - image_hand_weights.pixel(ev_y, ev_x).r;
//						image_hand_weights.pixel(ev_y, ev_x).b = temp;
	...
	output_file.close(); //fclose(output_file);	
}
